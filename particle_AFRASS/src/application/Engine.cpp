...
void Engine::update() {
  ...
  // réinitialise toutes les forces à appliquer à 0 :
  resetForce();
  ...
  if (_particleList->particleMode()==PARTICLE_SPHERE) interCollision();
...
void Engine::computeForce() {
  for(unsigned int i=0; i<_particleList->size(); i++) {
    ...
    if (p->alive()) {
      ...
      //Exercice2Question2 : poids = masse * gravité (cours chap7 p20), uniquement vers le bas
      double gravity = 9.81; //gravité terrestre
      p->addForce(Vector3(0.0, - p->mass() * gravity, 0.0));
